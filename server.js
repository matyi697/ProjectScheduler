...
app.get("/statusz/:munka_id", async (req, res) => {
  ...
  try {
    ...
    console.error(err);
    res.send("Hiba a státusz betöltésekor.");
  }
});
...
app.post("/uj-munka", async (req, res) => {
  ...
  try {
    ...
    console.error("Új munka mentési hiba:", err);
    res.send("Adatbázis hiba!");
  }
});
...
app.post("/statusz/:id", async (req, res) => {
  ...
  try {
    ...
    console.error("Státusz mentési hiba:", err);
    res.send("Hiba a mentéskor.");
  }
});
  ...
  try {
    ...
    const subJobRes = await client.query(
      ...
      [
        ...
        munkaAdatok.meres,
        munkaAdatok.arrival_date,
        munkaAdatok.due_date,
        munkaAdatok.notes,
      ],
    );
...
app.post("/api/munka-mentes", async (req, res) => {
  if (!req.session.userId)
    return res
      .status(401)
      .json({ success: false, error: "Nincs bejelentkezve" });
